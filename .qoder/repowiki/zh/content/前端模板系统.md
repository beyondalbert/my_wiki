...
## 更新摘要
**所做更改**
- 新增Excel/Spreadsheet在线表格编辑器集成章节，支持在线电子表格与Markdown富文本双模式编辑
- 更新条件渲染逻辑，通过doc.type判断文档类型，动态加载对应的编辑器和样式
- 新增JavaScript初始化代码，支持两种编辑器的条件加载和状态管理
- 更新文档服务层，支持content_json字段存储表格数据格式
- 新增多页面模板集成Excel编辑器的详细说明
- 更新大纲提取工具以支持新的内容格式
...
- 页面模板的布局与内容区块组织
- **新增** Excel/Spreadsheet在线表格编辑器集成与配置
- **更新** 条件渲染逻辑处理两种编辑模式（文档vs表格）
- **更新** JavaScript初始化代码支持新的编辑器
...
- **新增** Excel/Spreadsheet在线表格编辑器：提供Excel-like在线表格编辑体验，支持单元格操作、公式计算和数据验证。
- **新增** 条件渲染逻辑：通过doc.type判断文档类型，动态加载对应的编辑器和样式资源。
- **更新** Toast UI Editor富文本编辑器：提供WYSIWYG可视化编辑体验，支持Markdown语法和实时预览。
...
## 架构总览
模板系统遵循"基础模板 + 页面模板"的继承模式，页面模板通过 render_template 注入上下文变量，再由基础模板统一渲染输出。**新增** Excel/Spreadsheet在线表格编辑器提供Excel-like在线表格编辑体验，**更新** 条件渲染逻辑通过doc.type判断文档类型，动态加载对应的编辑器。**更新** 内容存储格式从Editor.js迁移到Markdown格式。**更新** Flash通知系统通过Alpine.js实现动画效果，交互式卡片设计增强用户界面体验。Markdown 与大纲工具在蓝图视图中被调用，为页面模板提供渲染后的 HTML 与导航信息。
...
**新增** Excel/Spreadsheet在线表格编辑器已成功集成到系统中，提供Excel-like在线表格编辑体验：
...
### 条件渲染逻辑
**新增** 条件渲染逻辑通过doc.type判断文档类型，动态加载对应的编辑器和样式：
...
- **样式规范**
  ...
  - **新增** 编辑器样式：Excel编辑器圆角边框设计
  - **新增** 条件样式：根据文档类型动态应用不同的样式类
- **响应式设计**
...
- 静态资源优化
  ...
  - **新增** Excel/Spreadsheet编辑器采用一体化版本，避免依赖缺失问题。
  - **新增** 条件加载策略，仅加载当前文档类型的编辑器资源。
- 模板渲染
...
## 结论
My Wiki 的模板系统以基础模板为核心，配合蓝图层的数据准备与工具层的渲染/提取能力，实现了清晰的职责分离与良好的可维护性。**新增** Excel/Spreadsheet在线表格编辑器的集成显著提升了文档编辑体验，提供专业的Excel-like在线表格功能。**新增** 条件渲染逻辑通过doc.type判断文档类型，动态加载对应的编辑器资源，实现了文档与表格的统一管理。**更新** 内容存储格式从Editor.js迁移到Markdown格式，提升了性能和兼容性。**更新** 通过引入高级动画Flash通知系统和交互式卡片设计，显著提升了用户体验。通过 Alpine.js 动画系统、响应式样式与轻量前端交互，系统在功能与体验上达到平衡。建议后续完善页面模板与组件宏定义，进一步提升可复用性与一致性。
...
- 模板变量清单（示例）
  ...
  - **新增** 条件渲染：is_sheet(布尔值)
  - **新增** 文档类型：doc.type('doc'/'sheet')
- 建议

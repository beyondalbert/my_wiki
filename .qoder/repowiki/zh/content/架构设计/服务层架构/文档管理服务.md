...
## 更新摘要
**所做更改**
- 更新了文档删除服务逻辑部分，强调软删除功能通过doc_service和blueprints实现
- 修正了UI变化不影响服务层功能的说明
- 补充了分组管理相关的实现细节
- 更新了文档树结构管理的分组处理逻辑
...
- 功能要点
  ...
  - 仅返回未删除文档
  - 支持文档分组功能，文档可以归属到不同的分组中
- 复杂度
  - 时间复杂度 O(n)，空间复杂度 O(n)
- 关键实现路径
  - [list_kb_doc_tree:11-72](file://app/services/doc_service.py#L11-L72)

```mermaid
flowchart TD
Start(["开始"]) --> Query["查询 KB 下所有未删除文档"]
Query --> Groups["查询文档分组"]
Groups --> GroupMap["构建分组映射"]
GroupMap --> Bucket["按分组和父节点分桶"]
Bucket --> Build["递归构建树结构"]
Build --> Return["返回嵌套列表"]
```
...
- 内容更新流程
  ...
  - 返回更新后的大纲与更新时间
- 关键实现路径
  - [create_document:84-102](file://app/services/doc_service.py#L84-L102)
  - [update_content:105-111](file://app/services/doc_service.py#L105-L111)
  - [蓝本保存接口:122-146](file://app/blueprints/doc.py#L122-L146)
...
participant O as "工具"
participant M as "模型"
C->>D : "POST /doc/{doc_id}/save {content_json,title,privacy,group_id}"
D->>S : "update_content(doc, content_json, title)"
S->>O : "extract_plain_text(content_json)"
O-->>S : "plain_text"
S->>M : "写入 content_json, plain_text, title, privacy, group_id"
M-->>S : "commit 成功"
S-->>D : "返回大纲与更新时间"
D-->>C : "JSON 响应"
```
...
- 删除策略
  ...
  - 后代收集使用广度优先遍历，确保完整清理
  - UI层面提供软删除确认对话框，明确告知删除影响范围
- 版本控制现状
  - 当前未实现历史版本存储；内容更新为覆盖式写入
- 关键实现路径
  - [collect_descendants:119-129](file://app/services/doc_service.py#L119-L129)
  - [软删除接口:149-162](file://app/blueprints/doc.py#L149-L162)
...
- 文档详情页同时渲染文档树与大纲
- 文档树由服务层构建，侧边导航由大纲驱动
- 支持文档分组显示，包括分组的展开/折叠、重命名、删除等功能
- 关键实现路径
  - [文档视图渲染:82-107](file://app/blueprints/doc.py#L82-L107)
  - [大纲提取:34-55](file://app/utils/outline.py#L34-55)

**章节来源**
- [app/blueprints/doc.py:82-107](file://app/blueprints/doc.py#L82-L107)
...
- 关键实现路径
  ...
  - [分享接口:165-186](file://app/blueprints/doc.py#L165-L186)
...
**图表来源**
- [app/blueprints/doc.py:165-186](file://app/blueprints/doc.py#L165-L186)
- [app/services/share_service.py:15-49](file://app/services/share_service.py#L15-L49)

...
**变更内容**
- 新增上传目录绝对路径解析功能的详细说明
- 增强了路径处理的健壮性和跨平台兼容性的技术细节
- 补充了上传目录安全验证和边界情况处理的说明
...
- 上传目录（UPLOAD_DIR）
  ...
  - 设置方式：通过环境变量覆盖
  - **新增功能**：绝对路径解析增强
    - 支持绝对路径：当配置为绝对路径时，直接使用 `Path(_upload_raw).resolve()` 进行解析
    - 支持相对路径：当配置为相对路径时，使用 `(BASE_DIR / _upload_raw).resolve()` 相对项目根目录解析
    - 跨平台兼容：统一使用 `pathlib.Path.resolve()` 进行路径规范化，确保在 Windows/Linux/macOS 上的一致性
    - 边界情况处理：自动处理路径中的 `..` 和 `.` 组件，防止路径遍历攻击
...
LoadCfg --> ParsePath["解析上传目录路径<br/>绝对/相对路径处理"]
ParsePath --> EnsureDirs["确保实例目录存在<br/>app/__init__.py"]
...
- 上传失败或超时
  ...
  - 查看服务器日志定位异常
- **新增**：上传目录路径问题
  - 检查 UPLOAD_DIR 配置是否为有效的绝对或相对路径
  - 确认路径解析后的实际目录是否存在
  - 验证路径中是否包含非法字符或路径遍历组件
- CSRF 校验失败
  - 确认表单中包含 CSRF 令牌
  - 检查 SameSite 与浏览器兼容性
- 登录状态异常
...
- **新增**：上传目录处理现已具备绝对路径解析功能，显著增强了路径处理的健壮性和跨平台兼容性
...
## 附录
- 环境变量与默认值
  - UPLOAD_DIR：默认 instance/uploads（支持绝对路径和相对路径解析）

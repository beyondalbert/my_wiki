...
## 更新摘要
**变更内容**
- 路由参数从<int:doc_id>更新为<doc_id>，反映文档ID从整数到字符串的迁移
- 数据模型中的文档ID类型从整数变更为12字符随机字符串
- ID生成机制采用无第三方依赖的nanoid风格算法
- 相关API文档中的参数类型和示例已同步更新
...
**更新** 新增Excel/Spreadsheet功能集成，支持双模式编辑系统（Markdown和Excel），替换原有Editor.js为Toast UI Editor，扩展前端模板以支持新的编辑器类型。文档ID从整数迁移到12字符随机字符串，提升URL友好性和安全性。
...
subgraph "模型层"
M1["文档 Document<br/>ID: String(12)"]
M2["知识库 KnowledgeBase<br/>ID: String(12)"]
M3["分享 DocumentShare"]
end
subgraph "工具层"
U1["安全工具 security"]
...
DocSvc->>Model : 创建Document记录<br/>ID : 12字符随机字符串
Model-->>DocSvc : 返回新文档
DocBP-->>Client : 重定向到编辑页
Client->>DocBP : POST /doc/<doc_id>/save
DocBP->>KB : can_edit(user, kb)
KB-->>DocBP : 允许/拒绝
...
**更新** 路由参数从<int:doc_id>更新为<doc_id>，因为文档ID现在是12字符随机字符串而非整数

**章节来源**
- [app/blueprints/doc.py:20-142](file://app/blueprints/doc.py#L20-L142)
- [app/__init__.py:71](file://app/__init__.py#L71)

#### 新建文档
- 方法：POST
- 参数：
  - kb_id：字符串，目标知识库ID（12字符随机字符串）
  - parent_id：可选字符串，父级文档ID（12字符随机字符串）
...
**章节来源**
- [app/blueprints/doc.py:44-58](file://app/blueprints/doc.py#L44-L58)
...
**章节来源**
- [app/blueprints/doc.py:61-69](file://app/blueprints/doc.py#L61-L69)
- [app/services/kb_service.py:26-36](file://app/services/kb_service.py#L26-L36)

#### 保存内容
- 方法：POST
- 路径参数：doc_id（12字符随机字符串）
- 请求体JSON字段：
  - title：可选字符串
...
**章节来源**
- [app/blueprints/doc.py:130-141](file://app/blueprints/doc.py#L130-L141)
...
- 文档ID生成：12字符随机字符串，使用无第三方依赖的nanoid风格算法

**更新** 文档ID从整数变更为12字符随机字符串，提升URL友好性和安全性
...
**更新** 路由参数从<int:doc_id>更新为<doc_id>，文档ID参数类型从整数更新为12字符随机字符串
...
- ID生成性能：12字符随机字符串生成开销极小，对性能影响可忽略
- URL友好性：字符串ID相比整数ID更友好，有利于SEO和分享
...
**更新** 文档ID从整数迁移到12字符随机字符串，提升了URL友好性、安全性与可扩展性，同时保持了API接口的向后兼容性。
...
- /doc/<doc_id>：GET，访问权限
- /doc/<doc_id>/edit：GET，编辑权限
- /doc/<doc_id>/save：POST，编辑权限
- /doc/<doc_id>/delete：POST，编辑权限
- /doc/<doc_id>/share：GET/POST，编辑权限
...
### ID生成机制
- 12字符随机字符串，使用无第三方依赖的nanoid风格算法
- 字符集：23456789abcdefghijkmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ（去除了易混淆字符）
- 碰撞概率：约5.8e20，对个人wiki量级远超足够
- 生成函数：generate_id(size=12) → str

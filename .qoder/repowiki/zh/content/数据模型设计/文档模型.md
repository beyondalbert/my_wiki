...
## 更新摘要
**所做更改**
- 更新了内容存储格式：从Editor.js JSON迁移到Markdown字符串
- 重构了大纲提取和纯文本处理逻辑为基于正则表达式的Markdown解析
- 更新了前端编辑器实现：从Editor.js切换到Toast UI Editor
- 更新了文档生命周期处理流程以适配新的内容格式
...
**更新** 本版本反映了从Editor.js到Toast UI Editor的重大迁移，content_json字段现在存储Markdown字符串，所有内容处理逻辑已重构为基于正则表达式的Markdown解析。

## 项目结构
围绕文档模型的关键模块与职责如下：
- 模型层：定义数据库表结构与关系（Document、DocumentShare、KnowledgeBase、User）
- 服务层：封装业务逻辑（文档树构建、内容更新、软删除、分享管理）
- 工具层：内容解析与渲染（Markdown内容解析、纯文本提取、Markdown渲染与安全过滤）
...
**更新** 编辑流程已从Editor.js JSON转换为直接的Markdown字符串处理。
...
- 查询优化
  ...
  - 大规模场景可考虑物化路径或邻接列表优化
- 存储与传输
  - **更新** Markdown字符串通常比Editor.js JSON更紧凑，减少存储空间占用
...
- **更新** Markdown字符串 + 纯文本提取，兼顾富文本与检索需求，相比Editor.js JSON更高效
- 安全渲染与白名单过滤，降低 XSS 风险
- 分享令牌与有效期控制，满足开放协作场景
- **新增** Toast UI Editor 集成，提供更好的富文本编辑体验

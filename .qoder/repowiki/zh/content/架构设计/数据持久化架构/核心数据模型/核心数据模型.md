...
## 架构总览
下图展示核心模型的类关系与外键约束，体现"用户-知识库-文档-AI知识库"的主干关系及"角色-权限"的授权关系，以及AI知识库内部的复杂关联。
...
## 结论
My Wiki 的核心数据模型围绕 RBAC 权限、知识库可见性与成员管理、以及富文本文档与分享机制展开。新增的AI知识库模型进一步完善了数据持久化架构，实现了Karpathy风格的LLM Wiki构建功能，支持智能文档转换、wikilink解析、双向链接网络和可选的RAG增强。通过明确的外键约束与级联策略、合理的索引与事务管理，系统在功能完备的同时兼顾了数据一致性与可维护性。建议在生产环境中强化连接池配置、引入缓存与分页、完善监控与日志，持续优化性能与稳定性。

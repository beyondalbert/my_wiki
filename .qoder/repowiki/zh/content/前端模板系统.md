...
## 更新摘要
**所做更改**
- 着陆页模板完全重写：从复杂的文档预览和公共知识库网格布局改为简洁的功能介绍和三步引导
- 新增六个核心功能展示区：知识库管理、文档与表格、AI双链Wiki、团队协作、安全分享、权限管控
- 新增三步工作流程：注册账号、创建知识库、编写与协作
- 文档视图界面重大重构：采用粘性头部设计，分离标题区域和内容区域
- 空内容状态处理改进：统一的空内容提示和编辑入口
- 滚动体验优化：增加scroll-margin-top补偿导航栏高度
- 大纲导航增强：改进平滑滚动和锚点处理机制
- 粘性定位操作区：知识库详情和文档详情页面采用一致的粘性定位设计
- 响应式布局优化：改进侧边栏和主内容区的网格布局
...
- 页面模板的布局与内容区块组织
- **更新** 着陆页模板完全重写：采用简洁的功能介绍和三步引导设计
- **更新** 新增六个核心功能展示区：知识库管理、文档与表格、AI双链Wiki、团队协作、安全分享、权限管控
- **更新** 新增三步工作流程：注册账号、创建知识库、编写与协作
- **更新** 文档视图界面重大重构：采用粘性头部设计，分离标题区域和内容区域
- **更新** 空内容状态处理改进：统一的空内容提示和编辑入口
- **更新** 滚动体验优化：增加scroll-margin-top补偿导航栏高度
- **更新** 大纲导航增强：改进平滑滚动和锚点处理机制
- **更新** 粘性定位操作区：知识库详情和文档详情页面采用一致的粘性定位设计
- **更新** 响应式布局优化：改进侧边栏和主内容区的网格布局
...
- **更新** 着陆页模板重写：采用简洁的功能介绍和三步引导设计，替代原有的复杂文档预览布局。
- **更新** 六个核心功能展示区：知识库管理、文档与表格、AI双链Wiki、团队协作、安全分享、权限管控，每个功能配有图标、标题和简要描述。
- **更新** 三步工作流程：注册账号、创建知识库、编写与协作，每步包含序号、标题和说明文字。
- **更新** 文档视图界面重构：采用粘性头部设计，分离标题区域和内容区域，提升用户体验。
- **更新** 空内容状态处理：统一的空内容提示和编辑入口，改善用户引导。
- **更新** 滚动体验优化：增加scroll-margin-top补偿导航栏高度，避免标题被遮挡。
- **更新** 大纲导航增强：改进平滑滚动和锚点处理机制，提供流畅的页面跳转体验。
- **更新** 粘性定位操作区：知识库详情页面和文档详情页面采用一致的粘性定位设计，确保操作按钮在滚动时保持可见。
- **更新** 响应式布局优化：改进侧边栏和主内容区的网格布局，提升不同屏幕尺寸下的显示效果。
...
## 架构总览
模板系统遵循"基础模板 + 页面模板"的继承模式，页面模板通过 render_template 注入上下文变量，再由基础模板统一渲染输出。Excel/Spreadsheet在线表格编辑器提供Excel-like在线表格编辑体验，条件渲染逻辑通过doc.type判断文档类型，动态加载对应的编辑器。内容存储格式从Editor.js迁移到Markdown格式。**更新** 通过引入重写的着陆页模板，显著提升了新用户的引导体验。**更新** 通过引入六个核心功能展示区，帮助用户快速了解产品价值。**更新** 通过引入三步工作流程，简化了用户入门路径。**更新** 通过引入重构的文档视图界面，显著提升了文档浏览体验。**更新** 粘性头部设计确保标题在滚动时保持可见，提升导航效率。**更新** 空内容状态处理提供了更好的用户引导。**更新** 滚动体验优化解决了导航栏遮挡标题的问题。**更新** 通过引入统一的粘性定位操作区，显著提升了知识库详情页面和文档详情页面的用户体验。**更新** 响应式布局优化确保在不同设备上的良好显示效果。Markdown 与大纲工具在蓝图视图中被调用，为页面模板提供渲染后的 HTML 与导航信息。

```mermaid
sequenceDiagram
participant U as "用户"
participant BP as "蓝图视图<br/>app/blueprints/main.py"
...
participant L as "着陆页模板<br/>app/templates/main/landing.html"
...
BP->>OL : 提取大纲/锚点
BP->>T : render_template("main/landing.html", 变量...)
T->>B : 继承基础模板并填充块
B->>F : 包含Flash组件
B->>L : 包含着陆页组件
B->>KB : 包含知识库详情组件
...
**图表来源**
- [app/blueprints/main.py:8-12](file://app/blueprints/main.py#L8-L12)
...
### 文档视图界面重构
**更新** 文档视图界面已完成重大重构，采用现代化的粘性头部设计：

- **粘性头部设计**
  - 位置：使用`sticky top-14`固定在导航栏(h-14)下方，确保标题在滚动时保持可见
  - 结构：分离标题区域和内容区域，提升视觉层次感
  - 样式：使用z-10确保标题层叠在内容之上，避免被内容遮挡
  - 类型标识：根据文档类型显示不同的颜色主题(#ecfdf5/#eef2ff)和图标

- **标题区域布局**
  - 标题：使用text-3xl字体大小，支持长标题换行(break-all)
  - 类型徽章：显示文档类型(📄 文档/📊 表格)，使用不同的颜色编码
  - 操作按钮：编辑和分享按钮，根据用户权限动态显示
  - 更新信息：显示最后更新时间和隐私状态

- **内容区域设计**
  - 空内容处理：统一的空内容提示和编辑入口
  - 文档模式：使用#viewer容器渲染Markdown内容
  - 表格模式：使用#sheet-viewer容器渲染Excel内容
  - 最小高度：内容区域设置min-height:600px确保良好显示效果

- **响应式布局**
  - 文档模式：主内容区使用col-span-9 lg:col-span-7
  - 表格模式：主内容区使用col-span-9 lg:col-span-10
  - 大纲导航：文档模式显示右侧大纲导航，表格模式隐藏

- **滚动体验优化**
  - scroll-margin-top: 5.5rem补偿导航栏高度，避免标题被固定在页面顶部的导航栏遮挡
  - 平滑滚动：点击大纲时使用scrollIntoView({ behavior: 'smooth', block: 'start' })
  - 首次加载：支持location.hash的精确定位，提供重试机制确保正确滚动

```mermaid
flowchart TD
DV["文档视图页面"] --> SH["粘性头部(sticky top-14)"]
SH --> TI["标题区域(title/h1)"]
TI --> TT["类型标识徽章"]
TI --> OB["操作按钮(编辑/分享)"]
SH --> UI["更新信息(last update/privacy)"]
DV --> MR["主内容区(col-span-9/10)"]
MR --> VC["内容容器(viewer/sheet-viewer)"]
MR --> ET["空内容提示(empty-tip)"]
DV --> GN["大纲导航(lg:block)"]
GN --> OA["大纲列表(outline)"]
```

**图表来源**
- [app/templates/doc/view.html:68-116](file://app/templates/doc/view.html#L68-L116)
...
**章节来源**
- [app/templates/doc/view.html:1-212](file://app/templates/doc/view.html#L1-L212)
...
- **更新** 粘性定位依赖**
  ...
  - col-span类：响应式网格布局
  - sticky top-14/20：粘性定位的具体位置
- **更新** 滚动体验依赖
  - scroll-margin-top：补偿粘性导航栏高度
  - scrollIntoView API：原生平滑滚动支持
  - requestAnimationFrame：确保DOM渲染完成后的操作
- **更新** 着陆页模板依赖
  - 网格布局：sm:grid-cols-2, lg:grid-cols-3
  - 卡片设计：rounded-xl, border, hover:shadow-md
  - 颜色编码：bg-indigo-50, bg-emerald-50等

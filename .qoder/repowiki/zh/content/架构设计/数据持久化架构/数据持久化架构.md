...
## 更新摘要
**变更内容**
- 新增AI知识库外部上传功能支持，包括数据库迁移脚本和相关字段
- 扩展AIKBSource模型以支持上传文件类型（UPLOAD）和相关存储字段
- 新增上传文件抽取工具类，支持PDF、Word、文本、图片等多种格式
- 完善AI知识库构建流程，支持混合来源（文档关联 + 上传文件）
- 更新数据库迁移机制以适配新的上传功能
...
5. [详细组件分析](#详细组件分析)
6. [ID系统迁移影响](#id系统迁移影响)
7. [AI知识库上传功能](#ai知识库上传功能)
8. [数据库迁移机制与版本管理](#数据库迁移机制与版本管理)
9. [依赖关系分析](#依赖关系分析)
10. [性能考量](#性能考量)
11. [故障排查指南](#故障排查指南)
12. [结论](#结论)
...
**更新** 新增AI知识库外部上传功能，支持PDF、Word、文本、图片等多种文件格式的上传和处理，扩展了AI知识库的构建能力。
...
EXTRACT_UPLOAD["上传抽取工具<br/>app/utils/extract_upload.py"]
RESET_SCRIPT["重置脚本<br/>scripts/reset_db.py"]
MIGRATE_SQL["迁移脚本<br/>scripts/migrate_aikb_source_uploads.sql"]
MIGRATE_PY["迁移脚本<br/>scripts/run_migrate_aikb_source_uploads.py"]
END
subgraph "外部依赖"
...
- AI知识库上传功能
  - 新增AIKBSourceKind枚举，支持DOCUMENT和UPLOAD两种类型。
  - 扩展AIKBSource模型，新增上传相关的存储字段。
  - 提供完整的上传文件抽取和处理工具。
...
- [app/utils/ids.py:1-21](file://app/utils/ids.py#L1-L21)
- [app/models/ai_kb.py:47-90](file://app/models/ai_kb.py#L47-L90)
- [scripts/reset_db.py:1-56](file://scripts/reset_db.py#L1-L56)
...
- 设计要点
  ...
  - **新增**：AIKBSourceKind枚举支持DOCUMENT和UPLOAD两种类型，扩展了上传文件的支持。
- 关系映射
...
- 外键与约束
  ...
  - 源文档与文章均使用唯一约束避免重复。
  - **新增**：AIKBSource的doc_id字段现在允许为NULL，支持上传文件类型。
- 查询与性能
...
+枚举 kind(DOCUMENT|UPLOAD,索引)
+字符串 doc_id(FK : documents.id,CASCADE,可空,索引)
+字符串 upload_filename
+字符串 upload_path
+字符串 upload_ext
+整数 upload_bytes
...
- **新增**：AI知识库上传功能迁移
  - 提供SQL脚本和Python脚本两种迁移方式，支持MySQL 5.7+和8.x。
  - 自动处理doc_id字段可空化、新增上传相关字段、创建索引等步骤。
  - 幂等执行，可重复运行而不产生错误。
...
- Wiki构建器
  ...
  - **新增**：支持上传文件类型的源文档处理，通过extract_text_from_upload工具抽取纯文本。
- 链接解析器
...
- 异步构建管道
  ...
  - **新增**：区分DOCUMENT和UPLOAD类型的源文档处理逻辑。
...
### 功能概述
My Wiki 新增了完整的AI知识库外部上传功能，支持多种文件格式的上传、处理和存储，扩展了AI知识库的构建能力。

### 支持的文件格式
- **文本文件**：.txt、.md、.markdown
- **PDF文件**：.pdf（使用pypdf库提取文本）
- **Word文档**：.docx（使用python-docx库提取文本和表格）
- **图片文件**：.png、.jpg、.jpeg、.webp、.gif、.bmp（通过多模态LLM进行OCR识别）

### 数据库模型扩展
AIKBSource模型新增了以下字段来支持上传功能：
...
### 上传处理流程
1. **文件接收**：通过AI蓝图的upload_source路由接收多文件上传
2. **文件验证**：检查文件扩展名是否在支持列表中
3. **文件保存**：生成唯一文件名并保存到服务器指定目录
4. **元数据记录**：在AIKBSource表中记录上传文件的元数据
5. **异步处理**：启动后台线程进行文件内容抽取和Wiki构建

### 文件抽取工具
`extract_upload.py`提供了统一的文件内容抽取接口：
...
B --> |文本文件| C[直接读取文本]
B --> |PDF文件| D[pypdf提取文本]
B --> |Word文件| E[python-docx提取文本和表格]
B --> |图片文件| F[多模态LLM OCR识别]
C --> G[返回纯文本]
D --> G
E --> G
F --> G
...
### 前端界面支持
AI知识库的源文档页面新增了上传功能区域，支持拖拽上传和多文件选择：

- **支持格式提示**：PDF / Word(.docx) / 文本(.txt/.md) / 图片(多模态OCR)
- **文件预览**：显示已选文件列表，支持逐个删除
- **实时大小计算**：显示文件大小，支持KB/MB格式化
- **批量上传**：支持多文件同时上传和处理

### 数据库迁移
为了支持上传功能，提供了两个迁移脚本：

1. **SQL脚本** (`migrate_aikb_source_uploads.sql`)：直接执行的SQL语句
2. **Python脚本** (`run_migrate_aikb_source_uploads.py`)：兼容MySQL 5.7+和8.x的自动迁移工具

迁移步骤包括：
- 将doc_id字段改为可空
- 新增kind、upload_filename、upload_path、upload_ext、upload_bytes字段
- 为kind字段创建索引
- 回填历史数据的kind字段
...
- 应用工厂在启动时将 db 实例与迁移实例绑定，确保迁移命令可用。
- 迁移实例默认使用 SQLAlchemy 元数据作为变更源，自动检测模型变化。
- **新增**：支持外部上传功能的迁移脚本，提供SQL和Python两种执行方式。

### 版本管理
- 通过 Flask-Migrate 的命令行工具生成与应用迁移脚本，保持数据库结构与模型同步。
- 建议在开发与生产环境中遵循"先迁移，后部署"的流程，确保一致性。
- **新增**：上传功能迁移脚本支持幂等执行，可重复运行而不产生错误。

### 数据一致性
- 外键约束与 CASCADE 删除策略在模型层面明确声明，配合数据库引擎保证参照完整性。
- 服务层在关键操作中使用 db.session 进行事务封装，避免部分写入导致的数据不一致。
- **新增**：上传功能的迁移脚本包含完整的回填逻辑，确保历史数据的完整性。
...
ALTER TABLE ai_kb_sources MODIFY COLUMN doc_id VARCHAR(12) NULL;

-- 2. 新增上传相关字段
ALTER TABLE ai_kb_sources ADD COLUMN IF NOT EXISTS kind VARCHAR(16) NOT NULL DEFAULT 'document';
ALTER TABLE ai_kb_sources ADD COLUMN IF NOT EXISTS upload_filename VARCHAR(255) NOT NULL DEFAULT '';
ALTER TABLE ai_kb_sources ADD COLUMN IF NOT EXISTS upload_path VARCHAR(500) NOT NULL DEFAULT '';
ALTER TABLE ai_kb_sources ADD COLUMN IF NOT EXISTS upload_ext VARCHAR(16) NOT NULL DEFAULT '';
ALTER TABLE ai_kb_sources ADD COLUMN IF NOT EXISTS upload_bytes INT NOT NULL DEFAULT 0;
...
- 外部依赖
  ...
  - **新增**：pypdf 3.0.0 支持PDF文件内容提取
  - **新增**：python-docx 0.8.11 支持Word文档内容提取
...
PYPDF["pypdf 3.0.0 (可选)"]
DOCX["python-docx 0.8.11 (可选)"]
CHROMA["chromadb (可选)"]
...
- 索引策略
  ...
  - **新增**：ai_kb_sources.kind索引，优化上传功能的查询性能。
...
- AI知识库问题
  ...
  - **新增**：上传文件处理失败时，检查文件格式支持、OCR模型配置和磁盘空间。
- ID系统问题
  - 若出现ID格式错误，检查ID生成器配置和使用方式。
  - 验证所有字符串ID参数传递是否正确，避免类型转换错误。
- **新增**：上传功能故障排查
  - 文件上传失败：检查文件大小限制、磁盘空间和文件权限
  - OCR识别错误：检查多模态LLM配置和图像质量
  - PDF提取失败：确认pypdf库版本和PDF文件有效性
  - Word文档处理错误：验证python-docx库安装和文档格式
...
**更新** 新增AI知识库外部上传功能，显著扩展了系统的文档处理能力。通过支持多种文件格式的上传、处理和存储，系统能够更好地适应多样化的知识管理需求。新增的迁移机制确保了向后兼容性，同时提供了灵活的部署选项。

建议在后续迭代中持续关注查询性能指标与索引覆盖率，并完善自动化测试与监控告警，以保障生产环境的稳定性与可维护性。同时，随着AI功能的不断完善，建议增加专门的AI知识库监控和性能优化策略，特别是针对上传文件处理的性能优化。
...
- 关键字段与索引建议
  ...
  - AI源文档：ai_kb_id、doc_id（唯一约束）、status（索引）、id（字符串主键）、kind（索引）
...
- **新增**：上传功能配置
  - AI_UPLOADS_DIR：上传文件存储目录
  - SUPPORTED_FILE_TYPES：支持的文件类型列表
  - MAX_FILE_SIZE：单文件最大大小限制
  - OCR_MODEL：多模态LLM模型配置
  - PDF_EXTRACTION：PDF文件提取配置
  - DOCX_EXTRACTION：Word文档提取配置
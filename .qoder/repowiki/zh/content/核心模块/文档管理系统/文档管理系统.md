...
- **新增：AI知识库集成**（Karpathy LLM Wiki风格），支持文档一键添加到AI知识库
...
- 数据模型
  ...
  - **新增：AI知识库模型**：支持名称、描述、聊天模型、RAG增强、状态管理、文章索引、链接解析
...
- 服务层
  ...
  - **新增：AI服务**：LLM客户端封装、Wiki构建器、链接解析器、异步构建管道、RAG聊天
...
- [app/models/knowledge_base.py:8-82](file://app/models/knowledge_base.py#L8-L82)
...
## 架构总览
系统采用"蓝图 + 服务 + 模型 + 工具"的分层架构，前后端通过 JSON API 交互，模板用于渲染页面。Markdown 渲染与大纲提取由工具模块完成，分享与权限控制由服务模块完成。**新增的图片上传功能通过专用路由处理，确保安全存储与访问控制。新增的AI知识库功能通过独立的蓝图和模型实现，支持Karpathy风格的LLM Wiki构建。**
...
Routes --> AI_Routes["AI知识库路由"]
Upload --> Storage["安全存储策略"]
AI_Routes --> AIBuild["Wiki构建管道"]
AIBuild --> LLM["LLM客户端"]
```
...
- [app/services/doc_service.py:11-81](file://app/services/doc_service.py#L11-L81)
- [app/services/kb_service.py:10-80](file://app/services/kb_service.py#L10-L80)
- [app/services/ai_service.py:47-444](file://app/services/ai_service.py#L47-L444)
- [app/services/share_service.py:15-49](file://app/services/share_service.py#L15-L49)
- [app/utils/markdown.py:31-87](file://app/utils/markdown.py#L31-L87)
...
- **新增：AI知识库模型**
  - 关键字段：名称、描述、聊天模型、RAG增强开关、状态（空闲/构建中/就绪/失败）、最后构建时间、错误信息
  - 关系：与用户的一对多关系；与文档的多对多关系（通过AIKBSource中间表）
  - 状态枚举：AIKBStatus（IDLE/BUILDING/READY/FAILED）
  - 源文档状态枚举：AIKBSourceStatus（PENDING/PROCESSING/PROCESSED/FAILED）
- 用户模型
...
User "1" -- "many" AIKnowledgeBase : "ai_kbs"
AIKnowledgeBase "1" -- "many" AIKBSource : "sources"
Document "1" -- "many" AIKBSource : "ai_sources"
...
**图表来源**
- [app/blueprints/doc.py:122-146](file://app/blueprints/doc.py#L122-L146)
...
**章节来源**
- [app/blueprints/doc.py:57-162](file://app/blueprints/doc.py#L57-L162)
...
### AI知识库集成与文档一键添加
- **新增：AI知识库功能**
  - **模型设计**：AIKnowledgeBase、AIKBSource、AIKBArticle、AIKBLink、AIKBChunk等模型
  - **状态管理**：支持空闲、构建中、就绪、失败四种状态
  - **构建流程**：基于Karpathy LLM Wiki方法论，将文档转换为Markdown条目
  - **链接解析**：支持双向超链接解析与关系图构建
- **新增：文档一键添加到AI知识库**
  - **界面集成**：在文档视图页面工具栏添加"AI知识库"下拉菜单
  - **功能实现**：支持用户选择AI知识库并一键添加文档
  - **状态反馈**：显示已加入的AI知识库，提供错误处理与进度提示
  - **自动构建**：添加后自动触发Wiki构建任务

```mermaid
flowchart TD
A["用户在文档视图点击 AI知识库"] --> B["显示下拉菜单"]
B --> C{"用户选择 AI知识库？"}
C --> |是| D["提交表单到 /doc/<doc_id>/add-to-ai-kb"]
C --> |否| E["关闭菜单"]
D --> F["服务端验证权限与存在性"]
F --> G{"文档已加入？"}
G --> |是| H["显示警告并返回"]
G --> |否| I["创建 AIKBSource 记录"]
I --> J{"AI知识库状态？"}
J --> |BUILDING| K["显示等待提示"]
J --> |其他| L["触发异步构建任务"]
K --> M["重定向回文档视图"]
...
**图表来源**
- [app/blueprints/doc.py:205-231](file://app/blueprints/doc.py#L205-L231)
- [app/blueprints/ai.py:92-169](file://app/blueprints/ai.py#L92-L169)
- [app/templates/doc/view.html:153-184](file://app/templates/doc/view.html#L153-L184)
...
- 文档相关
  ...
  - **新增**：POST /doc/<id>/add-to-ai-kb：将文档添加到AI知识库（需要登录）
- 知识库相关
...
- **新增：AI知识库相关**
  - GET /ai/：列出用户的所有AI知识库
  - GET/POST /ai/new：创建新的AI知识库
  - GET /ai/<ai_kb_id>：AI知识库详情页
  - GET /ai/<ai_kb_id>/sources：管理源文档
  - POST /ai/<ai_kb_id>/sources/add：批量添加源文档
  - POST /ai/<ai_kb_id>/build：触发Wiki构建
  - GET /ai/<ai_kb_id>/status：获取构建状态

**章节来源**
- [app/blueprints/doc.py:28-231](file://app/blueprints/doc.py#L28-L231)
...
- 外部依赖
  ...
  - **新增**：LLM客户端与向量化：openai、chromadb（可选）
  - 其他：Werkzeug、Pillow、requests
- 内部依赖
...
Services --> Models["模型"]
Models --> SQL
LLM["LLM客户端"] --> OpenAI["openai SDK"]
Vector["向量数据库"] --> Chroma["chromadb"]
```
...
- [app/services/doc_service.py:1-81](file://app/services/doc_service.py#L1-L81)
- [app/services/kb_service.py:1-80](file://app/services/kb_service.py#L1-L80)
- [app/services/ai_service.py:47-95](file://app/services/ai_service.py#L47-L95)
- [app/services/share_service.py:1-49](file://app/services/share_service.py#L1-L49)
...
- **新增：图片上传问题**
  ...
  - 访问失败：确认文件路径与安全文件名验证
- **新增：AI知识库构建问题**
  - API密钥配置：检查 OPENAI_API_KEY 是否正确配置
  - 构建失败：查看错误信息与日志，检查源文档内容
  - 状态异常：重置为待处理状态并重新触发构建
...
- [app/blueprints/ai.py:340-377](file://app/blueprints/ai.py#L340-377)

## 结论
本系统以清晰的分层架构实现了文档与知识库的核心能力：树形结构、富文本内容、权限控制与分享。通过 Editor.js JSON 与工具模块，系统具备良好的内容处理与渲染能力。**新增的图片上传功能完善了富文本编辑体验，通过严格的安全验证与存储策略确保系统稳定运行。最重要的新增功能是AI知识库集成，支持Karpathy风格的LLM Wiki构建，用户可以通过文档视图界面一键将文档添加到AI知识库，实现智能知识管理与问答。未来可在版本控制、移动/复制、缓存与分页等方面进一步增强。**
...
- [app/config.py:33-36](file://app/config.py#L33-L36)
- [app/services/ai_service.py:200-212](file://app/services/ai_service.py#L200-L212)
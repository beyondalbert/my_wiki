...
## 更新摘要
**变更内容**
- 新增AI服务组件的详细技术文档，涵盖LLM客户端封装、文章草稿构建、文章写盘、链接解析与回链、异步构建与再生、可选RAG对话
- 更新AI知识库模型架构，包括AIKnowledgeBase、AIKBSource、AIKBArticle、AIKBLink、AIKBChunk等实体
- 增强AI服务与蓝图的交互模式说明
- 补充AI服务的配置管理和外部依赖说明
...
M_AIKBSRC["模型: AI知识库源(AIKBSource)"]
M_AIKBART["模型: AI知识库文章(AIKBArticle)"]
M_AIKBLINK["模型: AI知识库链接(AIKBLink)"]
M_AIKBCHUNK["模型: AI知识库片段(AIKBChunk)"]
end
subgraph "基础设施"
...
### AI 服务(ai_service)
**更新** 新增AI服务组件，负责LLM客户端封装、文章草稿构建、文章写盘、链接解析与回链、异步构建与再生、可选RAG对话

职责与实现要点
- **LLM 客户端**：封装 OpenAI 兼容 SDK，支持多厂商与本地代理，通过配置管理 base_url、api_key、model。
- **文章构建**：将文档内容转为统一模板的 JSON 草稿，提取标题、别名、标签、摘要、内容与相关条目，支持安全长度限制。
- **文章写盘**：生成带 Front Matter 的 Markdown 文件，支持文件系统目录结构管理。
- **链接解析**：扫描 [[...]]，构建别名索引，生成正向与反向链接，支持红链检测。
- **异步构建**：后台线程批量处理源文档，更新状态与错误信息，支持增量重建。
- **可选 RAG 对话**：基于关键词匹配选择上下文进行问答，支持向量化检索增强。
...
- **外部依赖**：ai_service 依赖 openai SDK 与 slugify；captcha_service 依赖 Pillow；**ai_service 依赖 markdown 工具进行wikilink解析，依赖 outline 工具进行内容提取**；蓝图依赖 Flask-Login、CSRFProtect 等。
...
- **LLM 优化**：AI 服务通过温度参数控制生成稳定性，限制输入长度避免超出模型上下文，支持增量重建减少重复计算。
...
- **AI 构建失败**
  ...
  - **排查**：检查 LLM 凭据与模型配置；查看源文档是否存在且未删除；关注后台线程异常日志；确认 AI_WIKI_DIR 目录权限；检查 RAG 配置（如启用）。
  - **参考**
    - [app/services/ai_service.py:313-344](file://app/services/ai_service.py#L313-L344)
...
## 结论
服务层通过清晰的职责划分与严格的事务管理，将业务规则与数据访问解耦，提升了系统的可维护性与可测试性。配合蓝图的入口控制与模型层的数据契约，形成稳定可靠的分层架构。**新增的AI服务组件进一步增强了My Wiki的知识管理能力，通过LLM驱动的Wiki构建和智能问答功能，为用户提供更加智能化的文档处理体验。建议在现有基础上进一步完善单元测试与集成测试，强化边界条件与并发场景的覆盖，特别是AI服务的异步处理和外部依赖管理。**
...
- 依赖注入机制
  ...
  - 参考
    ...
    - [app/extensions.py:1-17](file://app/extensions.py#L1-L17)
- **AI 服务配置**
  - **LLM 配置**：通过 OPENAI_BASE_URL、OPENAI_API_KEY、CHAT_MODEL 环境变量配置，支持多种兼容 OpenAI API 的服务提供商。
  - **AI Wiki 目录**：AI_WIKI_DIR 指定生成的Markdown文件存储位置，默认为 instance/ai_wiki。
  - **RAG 配置**：ENABLE_RAG 开启向量化检索，EMBEDDING_MODEL 指定嵌入模型，CHROMA_PATH 指定向量数据库存储路径。
  - 参考
    - [app/config.py:37-47](file://app/config.py#L37-L47)
- 单元测试策略建议
  - 使用 pytest + SQLAlchemy 测试数据库，模拟蓝图请求与服务调用。
  - 针对服务层方法编写参数边界、异常分支与事务提交/回滚测试。
  - **针对 AI 服务使用 mock LLM 返回值与临时目录进行文件写入测试，模拟异步构建过程**。
  - 针对验证码服务使用固定种子与预设会话数据进行校验测试。
  - **针对 AI 服务的链接解析功能，准备包含wikilink的测试文档，验证别名索引和回链生成**。
  - 参考
    - [app/services/ai_service.py:313-344](file://app/services/ai_service.py#L313-L344)
    - [app/services/captcha_service.py:75-89](file://app/services/captcha_service.py#L75-L89)
    - [app/utils/markdown.py:68-87](file://app/utils/markdown.py#L68-L87)
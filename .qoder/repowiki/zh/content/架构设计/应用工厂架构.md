...
**更新** 本次更新反映了应用工厂架构新增搜索蓝图注册的重要变更，完善了Flask应用实例创建和蓝图注册流程，增强了全局文档搜索功能的完整性和一致性。
...
- 蓝图与路由：app/blueprints 下按功能域划分蓝图，如 main、auth、admin、kb、doc、share、ai、search 等。
...
- 蓝图注册
  - 注册 main、auth、user、admin、kb、doc、share、ai、search 等蓝图，并设置 url_prefix。
  - **新增** 搜索蓝图注册，提供全局文档搜索API，支持认证用户访问。
- 错误处理与上下文处理器
...
- [app/__init__.py:56-76](file://app/__init__.py#L56-L76)
- [app/__init__.py:78-98](file://app/__init__.py#L78-L98)
- [app/__init__.py:100-103](file://app/__init__.py#L100-L103)
...
### 蓝图注册与路由前缀
- 注册顺序与前缀
  - main 蓝图不带前缀，auth/admin/kb/doc/share/ai/search 各自带有 url_prefix。
  - **更新** 搜索蓝图注册顺序位于 share 蓝图之后，确保正确的路由优先级。
- 路由示例
  - main.index：根据登录状态跳转至仪表盘或首页。
  - auth.login/register/logout/captcha：认证相关路由。
  - admin：超级管理员后台，包含用户、角色、权限、公开内容管理等。
  - **新增** 搜索路由：/search 提供全局文档搜索功能，支持分页和权限控制。

**章节来源**
- [app/__init__.py:56-76](file://app/__init__.py#L56-L76)
- [app/blueprints/main.py:10-13](file://app/blueprints/main.py#L10-L13)
...
- [app/blueprints/search.py:11-107](file://app/blueprints/search.py#L11-107)
...
- 核心功能
  - 全局文档搜索：支持关键词搜索、分页、结果高亮
  - 权限控制：根据用户权限过滤可访问的知识库
  - 智能摘要：自动生成搜索结果摘要
  - 多条件查询：支持标题和内容双重匹配
...
- 组件耦合与职责分离
  ...
  - **新增** 搜索蓝图与AI知识库蓝图均通过工厂统一注册，确保架构一致性。
- 外部依赖
...
Search["blueprints/search.py"] --> Models["models/document.py"]
Search --> Models["models/knowledge_base.py"]
```
...
- [app/blueprints/search.py:70-85](file://app/blueprints/search.py#L70-L85)
...
- 蓝图路由 404
  ...
  - **新增** 检查搜索蓝图是否正确注册，确认/search路由可达。
- CLI 命令不可用
  - 确认工厂中已调用 register_cli(app)；在终端使用 flask init-db 或 flask create-user。
- **新增** 搜索功能异常
  - 检查数据库连接是否正常，确认Document和KnowledgeBase表结构完整
  - 验证用户权限是否正确，确认公共知识库可见性设置
  - 检查关键词长度是否至少2个字符
  - 查看数据库查询日志，确认LIKE查询和IN子句执行情况
- **新增** AI知识库功能异常
...
**更新** 本次更新完善了搜索蓝图的集成，通过应用工厂统一注册搜索蓝图，确保了全局搜索功能与现有架构的无缝衔接。新增的搜索API提供了强大的文档检索能力，支持权限控制和智能摘要，同时保持了系统的可维护性和扩展性。该架构易于扩展，可在工厂中新增初始化步骤，满足未来功能演进需求。
...
- 新增蓝图：在工厂中注册新蓝图，参考 [app/__init__.py:56-76](file://app/__init__.py#L56-L76)。
- 新增 CLI 命令：在 CLI 模块中注册命令，参考 [app/cli.py:61-96](file://app/cli.py#L61-L96)，并在工厂中调用 register_cli，参考 [app/__init__.py:100-103](file://app/__init__.py#L100-L103)。
- 新增错误处理或上下文处理器：参考 [app/__init__.py:78-98](file://app/__init__.py#L78-L98)。

### 搜索功能扩展指南
**新增** 以下路径展示了如何扩展搜索功能：
- 新增搜索索引：参考 [app/models/document.py](file://app/models/document.py) 的索引字段定义
- 新增搜索服务：参考 [app/services/search_service.py](file://app/services/search_service.py) 的搜索逻辑实现
- 新增搜索模板：参考 [app/templates/search/results.html](file://app/templates/search/results.html) 的结果展示
- 新增搜索路由：参考 [app/blueprints/search.py](file://app/blueprints/search.py) 的API接口定义

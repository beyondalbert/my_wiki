...
## 更新摘要
**变更内容**
- 更新管理员蓝图路由参数格式，从<int:id>统一到<id>格式
- 修正API接口文档中的路由参数类型说明
- 更新管理员API接口与管理工具使用指南中的参数格式
...
**更新摘要**
本次更新基于 Applied Changes：管理员蓝图角色和权限管理功能重大增强，包括新增角色编辑功能(edit_role)、权限创建和删除(new_permission/delete_permission)、改进的前端交互界面等。同时，更新了路由参数格式从<int:id>到<id>的统一变更。
...
### 管理员 API 接口与管理工具使用指南
**更新** 管理员 API 接口得到扩展，新增了角色编辑和权限管理相关接口。同时，更新了路由参数格式从<int:id>到<id>的统一变更。

...
## 更新摘要
**所做更改**
- 更新Flash通知系统章节，反映从简单静态显示到高级动画通知系统的升级
- 新增交互式卡片设计章节，详细说明知识库创建界面的改进
- 更新组件设计与样式规范，包含新的动画效果和视觉反馈
- 增强前端交互体验说明，涵盖Alpine.js和JavaScript集成
...
## 架构总览
模板系统遵循"基础模板 + 页面模板"的继承模式，页面模板通过 render_template 注入上下文变量，再由基础模板统一渲染输出。**更新** Flash通知系统通过Alpine.js实现动画效果，交互式卡片设计增强用户界面体验。Markdown 与大纲工具在蓝图视图中被调用，为页面模板提供渲染后的 HTML 与导航信息。

```mermaid
sequenceDiagram
participant U as "用户"
participant BP as "蓝图视图<br/>app/blueprints/kb.py"
...
U->>BP : 创建知识库
BP->>F : 发送Flash通知(success/error/info/warning)
F-->>U : 显示动画通知(4秒自动消失)
BP->>MD : 渲染Markdown/安全净化
BP->>OL : 提取大纲/锚点
BP->>T : render_template("kb/new.html", 变量...)
T->>B : 继承基础模板并填充块
B->>F : 包含Flash组件
F-->>U : 输出动画通知(颜色编码+动画)
...
**图表来源**
- [app/blueprints/kb.py:34-53](file://app/blueprints/kb.py#L34-L53)
...
## 结论
My Wiki 的模板系统以基础模板为核心，配合蓝图层的数据准备与工具层的渲染/提取能力，实现了清晰的职责分离与良好的可维护性。**更新** 通过引入高级动画Flash通知系统和交互式卡片设计，显著提升了用户体验。通过 Alpine.js 动画系统、响应式样式与轻量前端交互，系统在功能与体验上达到平衡。建议后续完善页面模板与组件宏定义，进一步提升可复用性与一致性。

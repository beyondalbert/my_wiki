...
本设计文档深入解析了My Wiki的蓝图系统，包括Flask蓝图的工作原理、URL前缀规则、路由组织策略、模板继承机制，以及各蓝图的功能职责和交互模式。
...
subgraph "蓝图依赖"
E[auth.py -> services/auth_service]
F[user.py -> services/kb_service]
G[admin.py -> models/* + utils/pagination]
H[kb.py -> services/kb_service + services/doc_service]
I[doc.py -> services/kb_service + services/doc_service + services/share_service]
J[ai.py -> services/ai_service + services/kb_service]
K[share.py -> services/share_service]
end
A --> E
A --> F
A --> G

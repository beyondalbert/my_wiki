...
本设计文档深入解析了My Wiki的蓝图系统，包括Flask蓝图的工作原理、URL前缀规则、路由组织策略、模板继承机制，以及各蓝图的功能职责和交互模式。特别关注新增的搜索蓝图，该模块实现了全局文档搜索功能，支持用户权限控制和知识库访问限制，为用户提供高效的文档检索体验。
...
**新增** 搜索蓝图提供全局文档搜索功能，支持用户权限控制和知识库访问限制。
...
F[ai_kb.py - AI模型]
G[knowledge_base.py - 知识库模型]
H[document.py - 文档模型]
I[user.py - 用户模型]
end
subgraph "蓝图依赖"
J[auth.py -> services/auth_service]
K[user.py -> services/kb_service + models/AIKnowledgeBase]
L[admin.py -> models/* + utils/pagination]
M[kb.py -> services/kb_service + services/doc_service]
N[doc.py -> services/kb_service + services/doc_service + services/share_service]
O[ai.py -> services/ai_service + services/kb_service + models/ai_kb]
P[share.py -> services/share_service]
Q[main.py -> services/kb_service]
R[search.py -> models/Document + models/KnowledgeBase + models/KBMember + models/KBVisibility + models/User]
end
A --> J
A --> K
A --> L
...
**新增** 搜索蓝图的性能优化策略：
1. **索引优化**：对搜索字段建立适当的数据库索引
2. **分页处理**：默认每页20条，最大50条，防止过度查询
3. **权限预计算**：预先计算用户可访问的知识库ID集合
4. **结果预加载**：批量预加载知识库名称，减少查询次数
5. **文本截取优化**：智能截取搜索结果片段，提升用户体验
...
**章节来源**
- [app/__init__.py:78-89](file://app/__init__.py#L78-L89)

## 结论

My Wiki的蓝图系统设计体现了现代Web应用的最佳实践，通过清晰的功能划分和严格的权限控制，实现了高度模块化的架构。各蓝图之间的协作通过统一的服务层和工具函数实现，确保了代码的可维护性和扩展性。

新增的搜索蓝图进一步完善了系统的实用性，提供了高效的全局文档搜索功能，支持用户权限控制和知识库访问限制，为用户提供了便捷的信息检索体验。该模块不仅增强了应用的功能完整性，也为未来的功能扩展奠定了坚实基础。

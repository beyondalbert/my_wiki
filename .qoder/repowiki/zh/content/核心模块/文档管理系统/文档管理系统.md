...
## 架构总览
系统采用“蓝图 + 服务 + 模型 + 工具”的分层架构，前后端通过 JSON API 交互，模板用于渲染页面。Markdown 渲染与大纲提取由工具模块完成，分享与权限控制由服务模块完成。
...
图表来源
- [app/blueprints/doc.py:69-85](file://app/blueprints/doc.py#L69-L85)
...
章节来源
- [app/blueprints/doc.py:20-101](file://app/blueprints/doc.py#L20-L101)
...
### 文件上传处理
- 当前实现
  - 图片块支持 URL 与 caption；未见专门的文件上传接口
- 建议
  - 在编辑器中集成上传组件，服务端生成安全文件名与存储路径
  - 返回相对 URL 供渲染使用，避免硬编码域名

章节来源
- [app/utils/outline.py:129-134](file://app/utils/outline.py#L129-L134)
...
## 结论
本系统以清晰的分层架构实现了文档与知识库的核心能力：树形结构、富文本内容、权限控制与分享。通过 Editor.js JSON 与工具模块，系统具备良好的内容处理与渲染能力。未来可在版本控制、移动/复制、文件上传、缓存与分页等方面进一步增强。
